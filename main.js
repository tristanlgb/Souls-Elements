...
let soulDate = solarDate(prompt("Ingrese su fecha de nacimiento para saber cuales son sus soul's elements, animals and flowers\nDe esta forma: x xx xxxx"));





function solaDate(inputDate) {
...
function elementSurrender(nextSoul){
    let mySoulyear = `${soulDate.getMonth() + 1} ${soulDate.getDate()} ${new Date().getFullYear()}`;
    let soulTest = new Date(mySoulyear) - new Date();
    let soulTest2 = Math.floor(soulTest / (1000 * 60 * 60 * 24)+1);
    
    if (soulTest2 > 0) {
        let nextSolar = `${soulDate.getMonth() + 1} ${soulDate.getDate()} ${new Date().getFullYear() }`;
        ...
        let nextSolar = `${soulDate.getMonth() + 1} ${soulDate.getDate()} ${new Date().getFullYear() +1 }`;
        let planetNumber = new Date(nextSolar) - new Date();
        let final = Math.floor(planetNumber / (1000 * 60 * 60 * 24)+1);   
        return final;
...
const salida = ` ${soulDate.getDate()} de ${month(soulDate)} de ${soulDate.getFullYear()}.\n

Te rodean ${elementsourrander(souldate)} elementos con tu misma vibracion. \n
